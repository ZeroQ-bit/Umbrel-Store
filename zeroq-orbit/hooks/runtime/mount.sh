...
case "${host_mount_parent}" in
  */data/zeroq-media) ;;
  *)
    echo "[entrypoint] refusing unexpected host media root ${host_mount_parent}" >&2
    exit 1
    ;;
esac
if ! mountpoint -q "${mount_parent}" 2>/dev/null; then
  echo "[entrypoint] refusing media root that is not a dedicated bind mount" >&2
  exit 1
fi
...
echo "[entrypoint] verified dedicated host storage safety"
